...
- **Downloads.** The qBittorrent and NZBGet queue, with pause, resume and remove. A setup checklist and one-click public indexers (Settings > Indexers) make sure requests actually find something. A few general-purpose public sources are connected automatically, so one being slow or blocked does not leave requests stuck.
