...
- **Downloads.** The qBittorrent and NZBGet queue, with pause, resume and remove. A setup checklist and one-click public indexers (Settings > Indexers) make sure requests actually find something. Every public, non-adult search source that answers is connected automatically (with a bundled FlareSolverr for Cloudflare-protected ones), so one being slow or blocked does not leave requests stuck.
