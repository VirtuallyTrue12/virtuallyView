...
export async function resolveAddTargets(
  ...
  // Music: an unknown name must not fall back to "Any", which accepts mono and low-bitrate rips.
  const profile = named ?? (service === 'lidarr' ? usable.find(p => /lossless/i.test(p.name!)) : undefined) ?? usable[0];
